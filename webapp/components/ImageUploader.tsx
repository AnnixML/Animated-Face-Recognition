...
const ImageUploader: FC<ImageUploaderProps> = ({ onUpload }) => {
    ...
    const handleDrop = useCallback(
        (event: React.DragEvent<HTMLDivElement>) => {
            event.preventDefault();
            const file =
                event.dataTransfer.files && event.dataTransfer.files[0]; // Check if there's a file
            if (file && !file.type.startsWith("image/")) {
                setError("Please select a valid image file.");
                return;
            }
    ...
    return (
        <div
            ...
            className="mx-auto size-1/2 rounded-lg shadow-xl upload-area border-2 border-dashed p-10 text-center py-2 px-4 rounded text-pl-3 border-2 border-rounded border-pl-3 bg-pl-2 dark:text-white dark:border-2 dark:border-rounded dark:border-pd-3 dark:bg-pd-2 justify-center"
